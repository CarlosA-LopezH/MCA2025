...
def fitness_std(generations: list[int], fitness_values: list[float], mean_value: list[float], std_values: list[float],
                name: str ="Task", subname: str = "Convergence", figure=None) -> None:
    """
    ...
    if figure is None:
        ...
        do_title = True
    figure.plot(generations, fitness_values, label="Best Fitness", color="green", linewidth=3)
    figure.plot(generations, mean_values, label="Mean Fitness", color="blue", linewidth=2)
    # Add shaded area for standard deviation
    figure.fill_between(generations, lower_bound, upper_bound, color='blue', alpha=0.3, label="Mean Fitness ± STD")
    # Labels and title
    figure.set_xlabel("Generations")
    figure.set_ylabel("Fitness Value")
    if do_title:
        plt.title(name)
        plt.legend()
        plt.show()
...
def fitness_min(generations: list[int], fitness_values: list[float], mean_value: list[float], min_value: list[float],
                name: str ="Task", subname: str = "Convergence", figure = None) -> None:
    """
    Plots the fitness function along with its standard deviation for each generation.
    :param generations: List of generations (x-axis).
    ...
    figure.plot(generations, fitness_values, label="Best Fitness", color="green", linewidth=3)
    figure.plot(generations, mean_values, label="Mean Fitness", color="blue", linewidth=2)
    figure.plot(generations, min_values, color='red', label="Min")
    # Labels and title
    figure.set_xlabel("Generations")
    figure.set_ylabel("Fitness Value")
    if do_title:
...
def visualize_liquid(lsm, name: str = "Liquid visualization", axes = None) -> None:
    """
    ...
    for connection in check_connections:
        ...
        t_pos = gids_pos[connection.connection]
        ...
        axes.annotate('', xy=t_pos, xytext=s_pos, arrowprops=dict(arrowstyle="->", color=type_c, alpha=0.5))
    # Identify excitatory neurons that receive input connection
    in_exc: set = set()
    for in_conn in conn_In:
        in_exc.add(in_conn.connection)  # Add target neuron to the set
    ...
    # Plot neurons
    axes.scatter(exc_in_x, exc_in_y, color='green', marker="s", label='Excitatory (Input connected)', s=100)
    axes.scatter(inh_x, inh_y, color='red', label='Inhibitory', s=100)
    ...
    if len(in_exc) != len(exc_gids):
        ...
        axes.scatter(exc_x, exc_y, color='green', label='Excitatory', s=100)
    # Add neuron GID labels for all neurons
    for gid, (x, y) in gids_pos.items():
        axes.text(x, y, f"{gid}", fontsize=8, ha='center', va='center', color="black")
