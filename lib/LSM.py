...
from lib.visualization import fitness_std, plot_input_data, plot_vm_separate, fitness_min, visualize_liquid, do_tsne, plot_spikes, exec_time
from lib.auxiliary import spike_rate
...
class LSM:
    ...
    neuron_model: str = 'iaf_psc_alpha' #'iaf_psc_delta'
    delay: float = 1.0  # This may be changed with future implementations

    def __init__(self, encoding: Encoding, inConn: float = 0.5, Ws: float = 200, neuronNoise_mean: float = 100.) -> None:
        # General parameters
        ...
        self.neuronNoise_std: float = abs(neuronNoise_mean / 10) # Standard deviation of the noise current amplitude (pA)
...
def validation(encoding: Encoding, lsm: LSM, data: dict, time_sim: float, cm=False) -> tuple[float, ndarray, list, list, ndarray]:
    """
    Validation process
    :param lsm: LSM net.
    :param data: Data.
    ...
    X_val = npArray(X_val)
    #   Get Readout results
    acc_val,_, cm = readout(X_val, data["Labels"], encoding.classifier, train=False, cm=cm)
